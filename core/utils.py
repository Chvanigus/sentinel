...
def split_file_name(layer_name: str) -> tuple:
    ...
    resolution = re.sub('[^0-9]', '',
                        names[6])  # Разрешение (по дефолту - 10м на пиксель)
    field_group_id = re.sub('[^0-9]', '', names[4]) if names[
                                                           4] != 'None' else None  # Номер Агро
...
def get_data_from_archive_sentinel(file_path: str) -> tuple:
    """ Разбивает строку названия файла на базовые составляющие
        :param file_path: Обрабатываемый архив
        :return: Кортеж строк
        :example:
    ...
    satellite = re.match(r'S[1-3][A-B]', name_file).group()
    date = datetime.strptime(
            re.search(
                    r'202\d{5}',
                    name_file
            ).group(), '%Y%m%d').date()
    tile = re.search(r'[A-Z]\d{2}[A-Z]{3}', name_file).group()
...
def unzip(src_path: str, dst_path: str) -> None:
    ...
    :param src_path: Путь к zip файлу

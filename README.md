...
полный набор опубликованных слоёв. Если tile-level TIFF уже сохранены в
`GS_DATA_ROOT`, повторный запуск восстанавливает их hardlink/copy и не
распаковывает SAFE и не пересчитывает индексы заново. Принудительный
`recalculate-ndvi` намеренно не использует этот кэш.
